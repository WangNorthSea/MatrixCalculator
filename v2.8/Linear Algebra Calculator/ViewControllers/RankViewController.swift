...
class RankViewController: UIViewController, KeyboardButtonClicked, UITextFieldDelegate, SavedMatricesDelegate {
    ...
    func buttonClicked(tag: Int) {
        if check {
            ...
            if tag == 13 {
                ...
                result = calculate.rank(calculate.A)
                screen.label1.text = screen.inputString
                screen.label2.text = "rank(A) = \(result)"
                check = false
            }
        }
        else {
