...
class TransposeViewController: UIViewController, KeyboardButtonClicked, UITextFieldDelegate, SavedMatricesDelegate {
    ...
    func buttonClicked(tag: Int) {
        if check {
            ...
            if tag == 13 {
                ...
                calculate.transpose()
                screen.inputString = calculate.matrixToString(calculate.B!)
                screen.label1.text = screen.inputString
                screen.label2.text = "Aᵀ = "
                check = false
            }
        }
        else {
