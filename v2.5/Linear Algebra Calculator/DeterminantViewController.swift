//
//  DeterminantViewController.swift
...
class DeterminantViewController: UIViewController, KeyboardButtonClicked, UITextFieldDelegate, SavedMatricesDelegate {
    ...
    @objc func decimalOrFraction() {
        ...
        if check || !decimalOrFractionCheck {
            ...
            decimalOrFractionForDetOrTrace()
            AudioServicesPlaySystemSound(1520)
            screen.label1.text = screen.inputString
        }
    }
    
    func decimalOrFractionForDetOrTrace() {
        screen.label2.text = calculate.decimalOrFractionForDetOrTrace(screen.label2.text!, 1)
    }
        ...
        if check {
            ...
            if tag == 13 {
                ...
                if screen.row == screen.firstLineColumn {
                    result = calculate.determinant(calculate.A)
                    screen.label2.text = "det(A) = \(result)"
                    check = false
                }
                else {
                    screen.label1.textColor = UIColor.red
                    screen.label1.text = "行列式要行列个数相等哦～！"
                    check = false
