...
class InverseViewController: UIViewController, KeyboardButtonClicked, UITextFieldDelegate, SavedMatricesDelegate {
    ...
    func buttonClicked(tag: Int) {
        if check {
            ...
            if tag == 13 {
                ...
                if screen.row == screen.firstLineColumn {
                    if !screen.inputString.contains("\n") {
                        //screen.inputString = calculate.inverse2(screen.inputString)
                        screen.matrixView.gridStringArray = calculate.inverse2(screen.inputString)
                        if !screen.matrixView.gridStringArray[0].contains("～") {
                            screen.matrixView.rows = screen.row
                            screen.matrixView.columns = screen.firstLineColumn
                            screen.matrixView.labelQuantity = screen.matrixView.rows * screen.matrixView.columns
                        ...
                        //screen.inputString = calculate.inverse()
                        screen.matrixView.gridStringArray = calculate.inverse()
                        if !screen.matrixView.gridStringArray[0].contains("～") {
                            screen.matrixView.rows = screen.row
                            screen.matrixView.columns = screen.firstLineColumn
                            screen.matrixView.labelQuantity = screen.matrixView.rows * screen.matrixView.columns
                            ...
                            screen.label1.text = screen.matrixView.gridStringArray[0]
                            screen.label1.textColor = UIColor.red
                            decimalOrFractionCheck = false
                        }
                    }
                    /*if screen.inputString != "您输入的矩阵不可逆～！" {
                        screen.label1.text = screen.inputString
                        screen.label2.text = "A⁻¹ = "
                    }
                    else {
