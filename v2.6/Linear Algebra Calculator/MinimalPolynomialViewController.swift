//
//  MinimalPolynomialViewController.swift
...
class MinimalPolynomialViewController: UIViewController, KeyboardButtonClicked, UITextFieldDelegate, SavedMatricesDelegate {
    ...
    func buttonClicked(tag: Int) {
        if check {
            if screen.column <= screen.firstLineColumn {
                ...
                if tag == 15 {
                    if screen.firstLineColumn == 5 && screen.row == 1 {
                        return
                    }
                    if screen.inputString.count > 0 {
                        if screen.inputString.last! == "." {
            ...
            if tag == 13 {
                ...
                if screen.row == screen.firstLineColumn {
                    ...
                    if calculate.minimalPolynomial() == "抱歉，您输入的矩阵太复杂了，本计算器算不了呢～！" {
                        screen.label1.textColor = UIColor.red
                        screen.label1.text = calculate.minimalPolynomial()
                        decimalOrFractionCheck = false
                    }
                    else {
                        screen.label2.text = calculate.minimalPolynomial()
                    }
                    check = false
                }
                else {
                    screen.label1.textColor = UIColor.red
                    screen.label1.text = "只有方阵才可以求极小多项式哦～！"
                    check = false
                    decimalOrFractionCheck = false
                }
            }
