...
class TraceViewController: UIViewController, KeyboardButtonClicked, UITextFieldDelegate, SavedMatricesDelegate {
    ...
    func decimalOrFractionForDetOrTrace() {
        screen.label2.text = calculate.decimalOrFractionForDetOrTrace(screen.label2.text!, 2)
    }
        ...
        if check {
            ...
            if tag == 13 {
                ...
                if screen.row == screen.firstLineColumn {
                    result = calculate.trace(calculate.A)
                    screen.label2.text = "tr(A) = \(result)"
                    check = false
                }
                else {
                    screen.label1.textColor = UIColor.red
                    screen.label1.text = "只有方阵才可求迹哦～！"
                    check = false
